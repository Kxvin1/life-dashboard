...
Revision ID: ai_insights_tables
...
revision: str = "ai_insights_tables"
...
def upgrade() -> None:
    # Check if ai_insight_usage table exists
    conn = op.get_bind()
    inspector = sa.inspect(conn)
    tables = inspector.get_table_names()

    if "ai_insight_usage" not in tables:
        ...
        op.create_index(
            op.f("ix_ai_insight_usage_date"), "ai_insight_usage", ["date"], unique=False
        )
        op.create_index(
            op.f("ix_ai_insight_usage_id"), "ai_insight_usage", ["id"], unique=False
        )
